...
def eval_epoch(encoder, decoder, dataloader, config):
    ...
    n = len(dataloader)
    return total_loss / n, total_iou / n, end_client-start_client, temp_time, end_server - start_server
...
def get_time(data):
    float_array = np.array(data, dtype=np.float32)
    bit = float_array.nbytes * 8  # 128 floats × 4 bytes × 8 = 4096 bits
    total_packet = bit / (1460 * 8)
...
if __name__ == "__main__":
    ...
    encoder_pth = torch.load("best_encoder_model_new.pth", map_location=config.device)
    encoder.load_state_dict(encoder_pth)

    decoder = Conv3DDecoder().to(config.device)
    decoder_pth = torch.load("best_decoder_model_new.pth", map_location=config.device)
...
print(avg_num/56)'''
with open('SC_time_AI_4voxel.csv', 'w', newline='') as file:
     writer = csv.writer(file)
     writer.writerow(enetime)

with open('SC_comtime_AI_4voxel.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(comtime)

with open('SC_decomtime_AI_4voxel.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(decomtime)

with open('SC_transtime_AI_4voxel.csv', 'w', newline='') as file:
    writer = csv.writer(file)
