...
GRID_SIZE   = 64
N_POINTS    = 4096          # sampled points per cloud (increase if you have GPU RAM)
LATENT_DIM  = 128
EPOCHS      = 00
LR          = 1e-3
DICE_W, BCE_W = 0.7, 0.3
...
def train_epoch(model, data, optimizer, bce_pos_weight, use_dice=True):
    ...
    for pts, vox, _, _ in data:
        ...
        optimizer.zero_grad()
        logits = model(x)
        loss = BCE_W * bce(logits, y) + (DICE_W * dice(logits, y) if use_dice else 0.0)
        loss.backward()
        optimizer.step()
...
def main():
    ...
    POS_WEIGHT = estimate_pos_weight(train_data)  # class-imbalance weight for occupied voxels
    ...
    for i, (pts, vox, fname,or_pts) in enumerate(test_data):
        ...
        loss = BCE_W * bce(logits, y) + DICE_W * dice(logits, y)
        total_loss += loss.item()
        ...
        original_size = np.prod(np.asarray(or_pts.points).shape)
    ...
    print(f"Test {i} | Average Loss {total_loss / n:.4f} Avgerage IoU {total_iou / n:.3f}")
